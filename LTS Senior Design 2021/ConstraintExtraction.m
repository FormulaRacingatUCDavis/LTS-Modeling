function [LAS, BSBS, BSBC, MMD, GGV] = ConstraintExtraction( PE )
...
%   MMD  - (struct) Base Speed Milliken Moment Diagram
%       .(field) - (n,m           numeric) Arbritrary Dynamic State
%
% Notes:
...
%% Input Parsing
if ~exist('PE', 'var')
    PE = load('TestPE.mat'); % Load Test Case Vehicle Data
end
...
for f = 1:numel(Field)
    LAS.(Field{f}) = permute( reshape( PE.(Field{f})(LAS.Ind), [], q, p ), [1 3 2] );
end
...
%% Base Speed Milliken Moment Diagram (MMD) Slicing
%%% MMD is found by slicing PE in both speed and longitudinal acceleration
for f = 1:numel(Field)
    MMD.(Field{f}) = reshape( PE.(Field{f})(PE.LongAcc==0 & PE.Speed==BaseSpeed), n, m );
end

%% GGV Diagram Rooting Finding
